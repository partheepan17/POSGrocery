...
interface CustomerCSVModalProps {
  onClose: () => void;
  onImport: () => void;
}
...
export function CustomerCSVModal({ onClose, onImport }: CustomerCSVModalProps) {
  ...
  const generatePreview = async (csvContent: string) => {
    ...
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        // Validate the row
        try {
          const customer_name = rowData.customer_name?.trim();
          const customer_type = rowData.customer_type?.trim() || 'Retail';

          // Basic validation
          if (!customer_name) {
            previewRow.status = 'error';
            previewRow.error = 'Customer name is required';
          } else if (!validTypes.includes(customer_type)) {
            previewRow.status = 'error';
            previewRow.error = `Customer type must be one of: ${validTypes.join(', ')}`;
          } else {
            // Check if customer already exists (this would be a database check in production)
            // For now, we'll simulate this
            const existingCustomers = ['John Doe', 'Jane Smith']; // Mock existing customers
            if (existingCustomers.some(name => name.toLowerCase() === customer_name.toLowerCase())) {
              previewRow.status = 'update';
              previewRow.warnings?.push('Will update existing customer');
            }
          }
  ...
  const handleImport = async () => {
    ...
    setLoading(true);
    try {
      const result = await csvService.importCustomers(csvData);
  ...
  const handleDownloadTemplate = () => {
    ...
    csvService.exportData(templateData, 'customers_template.csv');
            ...
            <div className="p-6">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600 mb-6">
                  Upload a CSV file with customer information. The file must include the required headers.
                </p>
                ...
                <div className="text-left bg-gray-50 rounded-lg p-4 mb-4">
                  ...
                  <ul className="text-sm text-gray-600 space-y-1">
                    <li><code className="bg-white px-1 rounded">customer_name</code> - Customer name (required)</li>
                  </ul>
                  ...
                  <ul className="text-sm text-gray-600 space-y-1">
                    ...
                    <li><code className="bg-white px-1 rounded">customer_type</code> - Retail, Wholesale, Credit, or Other (default: Retail)</li>
                    <li><code className="bg-white px-1 rounded">note</code> - Additional notes</li>
          ...
          {step === 'preview' && (
            <div className="p-6">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {previewRows.map((row) => (
                        <tr key={row.rowNumber} className={row.status === 'error' ? 'bg-red-50' : ''}>
                          ...
                          <td className="px-4 py-3 text-sm text-gray-900">{row.data.customer_name}</td>
                          <td className="px-4 py-3 text-sm text-gray-900">{row.data.phone || '-'}</td>
                          <td className="px-4 py-3 text-sm text-gray-900">
                            {row.data.customer_type && (
                              <span className={`px-2 py-1 text-xs rounded-full ${getTypeColor(row.data.customer_type)}`}>
                                {row.data.customer_type}
                              </span>
                            )}
                          </td>
              ...
              <div className="flex items-center justify-between mt-6 pt-6 border-t">
                ...
                >
                  {loading ? 'Importing...' : `Import ${validRowsCount} Customers`}
                </button>
              </div>
            </div>
          )}
