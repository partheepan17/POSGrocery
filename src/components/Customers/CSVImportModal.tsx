...
export function CSVImportModal({ onClose, onImport }: CSVImportModalProps) {
  ...
  const generateImportPreview = async (csvData: any[]): Promise<ImportPreview> => {
    ...
    for (let i = 0; i < csvData.length; i++) {
      ...
      if (!rowData.customer_name) {
        row.errors.push('Customer name is required');
      }
      ...
      const numericFields = ['credit_limit', 'discount_percent'];
      ...
      if (row.errors.length > 0) {
        ...
        try {
          const existingCustomer = await dataService.getCustomerByPhone(rowData.contact_phone);
          if (existingCustomer) {
            row.status = 'updated';
            row.existingCustomer = existingCustomer;
            updatedCount++;
          } else {
            row.status = 'new';
            newCount++;
          ...
          // Customer doesn't exist, so it's new
          row.status = 'new';
          newCount++;
        }
      }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      for (const row of preview.rows) {
        ...
        try {
          const customerData = {
            ...
            customer_type: (row.data.customer_type || 'Retail') as 'Retail' | 'Wholesale' | 'Credit' | 'Other',
            note: row.data.note || '',
          ...
          if (row.status === 'new') {
            ...
            await dataService.updateCustomer(row.existingCustomer!.id, customerData);
          }
      ...
      toast.success(`Import completed: ${successCount} customers imported, ${errorCount} errors`);
      onImport();
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-6xl w-full mx-4 max-h-[90vh] overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Import Customers from CSV</h2>
          <button
            onClick={onClose}
            ...
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Select a CSV file with customer data to import
                </p>
              </div>
              ...
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <div className="text-sm text-blue-800">
                  ...
                  <button
                    onClick={() => downloadTemplate(getTemplateConfig('customers'))}
                ...
                <>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-blue-50 p-3 rounded-lg">
                      ...
                      <div className="text-sm text-blue-800">Updated Customers</div>
                    </div>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center space-x-2">
                              <span className="text-sm font-medium text-gray-900">
                                Row {row.row}: {row.data.customer_name || 'No Name'}
                              </span>
        ...
        {preview && (
          <div className="flex items-center justify-end space-x-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
            ...
            >
              {importing ? 'Importing...' : `Import ${preview.summary.new + preview.summary.updated} Customers`}
            </button>
          </div>
        )}
      </div>
