...
export function CSVImportModal({ onClose, onImport }: CSVImportModalProps) {
  ...
  const generateImportPreview = async (csvData: any[]): Promise<ImportPreview> => {
    ...
    for (let i = 0; i < csvData.length; i++) {
      ...
      if (!rowData.supplier_name) {
        row.errors.push('Supplier name is required');
      }
      if (!rowData.contact_phone) {
        row.errors.push('Contact phone is required');
      }

      // Validate email format
      if (rowData.contact_email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(rowData.contact_email)) {
        row.errors.push('Invalid email format');
      }

      // Validate payment terms
      if (rowData.payment_terms && isNaN(parseInt(rowData.payment_terms))) {
        row.errors.push('Payment terms must be a valid number');
      }
      ...
      if (row.errors.length > 0) {
        ...
        try {
          const existingSupplier = await dataService.getSupplierByName(rowData.supplier_name);
          if (existingSupplier) {
            row.status = 'updated';
            row.existingSupplier = existingSupplier;
            updatedCount++;
          } else {
            row.status = 'new';
            newCount++;
          ...
          // Supplier doesn't exist, so it's new
          row.status = 'new';
          newCount++;
        }
      }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      for (const row of preview.rows) {
        ...
        try {
          const supplierData = {
            ...
            contact_email: row.data.contact_email || '',
            address: row.data.address || '',
            tax_id: row.data.tax_id || '',
            payment_terms: parseInt(row.data.payment_terms) || 30,
          ...
          if (row.status === 'new') {
            ...
            await dataService.updateSupplier(row.existingSupplier!.id, supplierData);
          }
      ...
      toast.success(`Import completed: ${successCount} suppliers imported, ${errorCount} errors`);
      onImport();
    } catch (error) {
  ...
  const handleDownloadTemplate = () => {
    const templateData = [
      ...
      },
      {
        ...
        contact_email: 'info@xyztrading.com',
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-6xl w-full mx-4 max-h-[90vh] overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Import Suppliers from CSV</h2>
          <button
            onClick={onClose}
            ...
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Select a CSV file with supplier data to import
                </p>
              </div>
              ...
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <div className="text-sm text-blue-800">
                  ...
                  <button
                    onClick={() => downloadTemplate(getTemplateConfig('suppliers'))}
                ...
                <>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-green-50 p-3 rounded-lg">
                      ...
                      <div className="text-sm text-green-800">New Suppliers</div>
                    </div>
                    <div className="bg-blue-50 p-3 rounded-lg">
                      <div className="text-2xl font-bold text-blue-600">{preview.summary.updated}</div>
                      <div className="text-sm text-blue-800">Updated Suppliers</div>
                    </div>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center space-x-2">
                              <span className="text-sm font-medium text-gray-900">
                                Row {row.row}: {row.data.supplier_name || 'No Name'}
                              </span>
        ...
        {preview && (
          <div className="flex items-center justify-end space-x-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
            ...
            >
              {importing ? 'Importing...' : `Import ${preview.summary.new + preview.summary.updated} Suppliers`}
            </button>
          </div>
        )}
      </div>
